...
读取顺序：`docs/status.md` → `README.md` → `SPEC.md` → `docs/decisions/README.md` → 当前 run 的 `evaluation.md`。
...
- 每次新的实现/选择决策写入 `docs/decisions/<decision-name>-<YYYYMMDD-HHmmss>.md`，不覆盖旧决策。
- 没有三个真实 run 时不得生成最终赢家；stub/MockTransport 只证明接口合同。
